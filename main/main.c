...
int16_t coords_mem_offset[COORDS_MAX_OFFSET][COORDS_COLS] = {{0, 0, 0}};
int16_t coords_max[COORDS_COLS] = {0, 0, 0};
int16_t curr_row = 0;
int16_t coords_mem[COORDS_ROWS][COORDS_COLS] = {{0, 0, 0}};
...
int main(void) {
  ...
  for (;;) {
    ...
    if (HAP_CS2_PORT == 1 && f_timer == 1) {
      f_timer = 0;
      res = wsen_itds_get_all_coords2(wsen_itds_driver, coords);
      if (!res) {
