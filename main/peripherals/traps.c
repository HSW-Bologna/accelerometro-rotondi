...
void (*getUpLoc(void))(void);      // Get Address Error Loc
...
void __attribute__((interrupt, no_auto_psv)) _StackError(void)
{
    ...
    // errLoc = getUpLoc();	// get the upper func of error func

    INTCON1bits.STKERR = 0;     // Clear the trap flag

    // errLoc();	// return the function you selected above.
